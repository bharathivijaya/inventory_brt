    ...
    public function generate()
	{
        ...
        if (!@empty($_POST['parameters']))
		{
            $data['labels'] = array(
                ...
                'dname' => 'Drug Name',
				'deleted' => 'Deleted',
				'e_deleteddate' => 'Date Deleted',
				'e_last_edit_date' => 'Last Edited On',
            );

            $data['dates'] = array('e_date', 'e_expiration', 'd_modified', 'd_created', 'd_modified','e_deleteddate');
		...
		$from_date 				 = strtotime($date1);
		$to_date   				 = strtotime($date2)+24*60*60;
	
		$search_query 			 = $this->input->post('search_transaction'); 

			$this->session->set_userdata('search_query',$search_query);
		$this->session->set_userdata('date1',$from_date);
		$this->session->set_userdata('date2',$to_date);		
	
						$data['entries'] = $this->reportM->deleted_transaction_report($from_date,$to_date,$search_query,$cuser['id']);
     ...
     public function create_pdf(){
        ...
        if ($_POST['pb'] == 'no') {
            $content = $match[1];
						...
						$str="<input type=\"submit\" value=\"View Transaction\" class=\"btn btn-primary btn-xs\">";
