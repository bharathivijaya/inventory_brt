  ...
  <table class="table table-striped table-hover entry-table" style="border:1px solid #f9f9f9;margin-top:0px;margin-bottom:0px;">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?= $original_transaction[0]->e_type == 'new' ?'IN' : strtoupper($original_transaction[0]->e_type) ?></td>
...
<h3> Related Transactions - Edited Entries</h3>
  <table class="table  table-hover dt-responsive"  style="width:100%;border:0px solid #FFFFFF;margin-top:0px;margin-bottom:0px;" cellspacing="0" id="transTable">
      ...
      <tr>
        ...
        <td colspan=3>Total Quantity Out<!--<?= $original_transaction[0]->e_type == 'new' ? 'In' : 'Out (Edit)' ?>--></td>
        <td colspan=3>Username</td>
      </tr>
    </thead> <tbody>  <tr>
        <td colspan=2><?= date('m-d-Y H:m:s', $entry->e_last_edit_date);?>
        </td>
        <td colspan=2> <?= $entry->e_type == 'new' ?'IN' : "OUT (".strtoupper($entry->e_type).")" ?></td>
  ...
  <tr>
      ...
      <tr class="units">
        ...
        <td><?php echo $entry->d_size;/* if ($entry->d_size == 0) {
                echo 0;
            } else {
...
<td>    <input  type="submit" value="Reverse Transaction" onclick="confirmDialog('<?php echo $transaction_id; ?>');" class="btn btn-primary"/>      </td>
     <?php if(isset($_POST['fromdashboard']) && $_POST['fromdashboard']==1) { ?><td><input  type="submit" value="Back" onclick="backButton(1);" class="btn btn-primary"></td>
...
<div class="row hidden" id="table2">
  <form action="<?=base_url()?>edit_transaction/edit_entry/<?php echo $transaction_id; ?>" method="post" class="save-form" id="form1">
    ...
    <div style="width: 80%;padding-bottom:20px;">
      ...
      <div class="row"> 
       <div class="col-md-4"> <span id="lots_total_count" style="text-align: right; padding: 5px;margin-bottom:20px;"><b>New Lot Total: 0</b></span></div>
...
<script>
	...
	function addNewLotsMulti()
	{
		...
		'<td><select ' +  select_lot_ddl + ' class="form-control" id="lot_select_' + LID + '"><option>'+lot_name+'</option></select>'+
...
var LOTS2 = <?= json_encode($lots_data) ?>;
	...
	function addNewLotsbrt(ch)
	{
		...
		$(to_field).append('<tr id="lotrow_' + LID + '">'+'<td><select name="lotnobrt_'+LID+'" id="lotnobrt_'+LID+'" class="form-control" onchange="checkalready('+LID+');"><option value="" selected disabled>Select</option><option value="newlot">Add New Lot</option><?php foreach($lots_data as $lot) { if($drug["d_id"]==$lot["drugId"])  { ?><option value="<?php echo $lot["lotName"]; ?>"><?php echo $lot["lotName"];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo date("m-d-Y",$lot["expirationDate"]);?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $lot["count"];?><?php } } ?></td><span style="font-size: 85%; color: #B94A48;"></span>		<b id="elot_warning_' + LID + '" style="display: none; color: #FF0000;">Warning! You are adding to an existing Lot Lumber in the system</b>' +		'<td><input type="text" id="edate_' + LID + '" name="audit_expiration[]" class="e_date form-control datetimeST3" value="' + e_date + '" required onchange="setqoh(' +LID+');"/><span style="font-size: 85%; color: #B94A48;"></span></td>' +
	...
	function addNewLots()
	{
		...
		'<td><select '+ select_lot_ddl +' class="form-control" id="lot_select_' + LID + '" onchange="showdetails(' + LID + ')";><option>'+lot_name+'</option><?php foreach($lots_data as $lot) { if($drug["d_id"]==$lot["drugId"])  { ?><option value="<?php echo $lot["lotName"]; ?>"><?php echo $lot["lotName"];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo date("m-d-Y",$lot["expirationDate"]);?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $lot["count"];?></option><?php } } ?></select><input type="hidden" lid="' + LID + '" drug-id="<?= @$drug["d_id"] ?>" id="elot_' + LID + '" name="audit_lot[]" value="' + e_lot + '" required/> <input type="hidden" id="e_lot_original_' + LID + '" name="original_lot[]"  value="' + lot_number + '" />   </td>'+ '<td><input type="text" id="edate_' + LID + '" name="audit_expiration[]" class="form-control" readonly value="' + e_date + '" required/></td>' +
	...
	alert(lid);
		var lotcode=$('#lot_select_'+lid).val();
	alert(lotcode);
	
	var ret=checkalready(lotcode,lid);
if(ret==1) { checkLotNumber(lotcode,lid);
for (var i = 0; i < LOTS2.length; i++) {
	//alert("select lot name:"+LOTS2[i].lotName);
	//alert("lid val:"+lotcode);
			if (LOTS2[i].lotName == lotcode) {
				$("#lot_select_" + lid).val(LOTS2[i].lotName);
	...
	function checkalready(lotcode,lid)
	{
