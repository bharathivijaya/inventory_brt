 ...
 public function bulkuploadqtyin($file, $cid)
	{
                  ...
                  /*  for ($i = 0; $i <= 2; $i++)
                    ...
                    foreach ($qtyins as $one)
					{
                        ...
                        if (isset($qtyin[1]))
						{
                            ...
                            if (!($this->checkLotExpdate($post['in_lotname'], $post['in_expdate'])))
							{
								$lotnos[] = $post['in_lotname'];
                                //$drugId = $this->addDrug($post);
								
                            } 
							$this->lotM->bulkuploadqtyin($post);
                            $answer = 'ok';
                        }
                    }
                }
        ...
        $arr = array('answer' => $answer, 'lotnos' => $lotnos);
        return $arr;
    }
 public function checkLotExpDate($lotcode, $expdate) {
        $lot = $this->lotM->getLotDetails( $lotcode, $expdate);
